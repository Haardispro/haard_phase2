...
Which is just normal RSA. What we have is normal ciphertext C. Now we can choose a plaintext, so lets say we choose 2 as our plaintext. We can now compute `Ca = 2^e mod n`. However to our program (victim) we send `Cb = Ca * C`, so now : 
...
Final flag: `picoCTF{su((3ss_(r@ck1ng_r3@_881d93b6}`
